    ...
    dmc.Modal(
        ...
        children=[
            dmc.Stack([
                ...
                # Dynamic Client Name Fields Container
...
# Callback to open add client modal
@callback(
...
def toggle_add_modal(open_clicks, cancel_clicks, submit_clicks, is_opened):
    """Toggle add client modal"""
    triggered_id = ctx.triggered_id

    if triggered_id == "add-client-button":
        return True
    elif triggered_id in ["cancel-add-client", "submit-add-client"]:
        return False

    return is_opened


# Callback to update client name fields dynamically based on client type
@callback(
    Output("client-name-fields-container", "children"),
    Input("new-client-type", "value")
)
def update_client_name_fields(client_type):
    """Show different input fields based on client type"""
    if not client_type:
        return dmc.Alert(
