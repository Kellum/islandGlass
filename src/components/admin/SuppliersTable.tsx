...
export function SuppliersTable({ onToast }: SuppliersTableProps) {
  ...
  const load = async () => {
    try {
      ...
      onToast('error', 'Failed to load suppliers');
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      await updateSupplier(editingId, editName.trim());
      onToast('success', 'Supplier updated');
      setEditingId(null);
      load();
    } catch {
      onToast('error', 'Failed to update supplier');
    }
  };

  const handleDelete = async (id: number) => {
    if (!confirm('Delete this supplier? Any glass configs using it will lose their supplier assignment.')) return;
    try {
      await deleteSupplier(id);
      onToast('success', 'Supplier deleted');
      load();
    } catch {
      onToast('error', 'Failed to delete supplier. It may be in use by glass configs.');
    }
  };

  const handleCreate = async () => {
    if (!newName.trim()) {
      onToast('error', 'Supplier name is required');
      return;
    }
    try {
      await createSupplier(newName.trim());
      onToast('success', 'Supplier added');
      setShowAddForm(false);
      setNewName('');
      load();
    } catch {
      onToast('error', 'Failed to add supplier. Name may already exist.');
    }
  };
      ...
      <AnimatePresence>
        {showAddForm && (
          ...
          >
            <Card>
              <CardContent>
                <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-3">New Supplier</h4>
                <div className="flex gap-3">
                  ...
                  <Button size="sm" variant="secondary" onClick={() => { setShowAddForm(false); setNewName(''); }}>Cancel</Button>
                </div>
              </CardContent>
            </Card>
          </motion.div>
